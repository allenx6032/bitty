...
public:
	...
	virtual void open(const class Project* project, const char* name, Object::Ptr obj, const char* /* ref */) override {
		...
		_object = Object::as<Text::Ptr>(obj);
		if (_object) {
			size_t len = 0;
		...
		SetModifiedHandler(std::bind(&EditorTextImpl::modified, this));

		fprintf(stdout, "Text editor opened: \"%s\".\n", _name.c_str());
	}
	virtual void close(const class Project* project) override {
		...
		fprintf(stdout, "Text editor closed: \"%s\".\n", _name.c_str());
...
EditorText* EditorText::create(void) {
	EditorTextImpl* result = new EditorTextImpl();
